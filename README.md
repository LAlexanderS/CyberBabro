...
2. Создание локального виртуального окружения:
   - Перейти в корневую папку, где находится malomobil.
...
4. Перейти в папку проекта:
   - Выполнить команду: `cd ./malomobil
...
6. Установка всех пакетов. Находимся в директории malomobil (где лежит файл `requirements.txt`):
   - Выполнить команду: `pip3 install -r requirements.txt`
7. Создаем `.env` в корне по образцу `.env.example`.
8. Запуск проекта. Находимся в директории malomobil (где лежит файл `manage.py`):
...
5. Переходим в папку проекта malomobil.
...
1. Создаём миграцию для определения модели БД:
   - Выполнить команду: `python3 manage.py makemigrations`
